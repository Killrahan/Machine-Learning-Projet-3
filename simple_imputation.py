"""
This file uses simple imputation as only feature engineering
"""
        ...
        for f in f_indexes:
            ...
            for line_index in subject_indexes[i]:
                X[i][:, (index)*512:(index+1) *
                     # set_stat(data_array[index][int(line_index)])
                     512][k] = data_array[index][int(line_index)]
                k += 1

            for line in range(3500):
                X_test[:, (index)*512:(index+1) *
                       # set_stat(data_test_array[index][line])
                       512][line] = data_test_array[index][line]
